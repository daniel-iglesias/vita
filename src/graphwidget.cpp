...
#ifdef USE_QCUSTOMPLOT
GraphWidget::GraphWidget(const QString& title, QWidget* parent, QCustomPlot* inputPlot)
{
  theWidget = new QMainWindow;
  theGraph = new QLabel;
//   theGraphMap = new QPixmap;
  this->setWidget( theWidget );
  theWidget->setCentralWidget( inputPlot );

//   this->resize(this->width(), parent->height()*.4);
//  this->move(0,0);
  theWidget->show();
